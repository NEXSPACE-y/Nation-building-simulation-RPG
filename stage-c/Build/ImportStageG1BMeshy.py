...
MESH_NAME = "SK_PLAYER_M_Meshy_v0_1"
SKELETON_NAME = "SKEL_PLAYER_M_Meshy_v0_1"
PHYSICS_NAME = "PHYS_PLAYER_M_Meshy_v0_1"


def log(message: str) -> None:
    unreal.log(f"STAGE_G1B_IMPORT {message}")
...
def imported_height(mesh) -> float:
    for property_name in ("imported_bounds", "positive_bounds_extension"):
        try:
            value = mesh.get_editor_property(property_name)
            if property_name == "imported_bounds":
                return float(value.box_extent.z * 2.0)
        except Exception:
            pass
    try:
...
def configure_texture(path: Path, name: str, srgb: bool, compression):
    ...
    if texture is None:
        raise RuntimeError(f"Texture import failed: {name}; imported={task.imported_object_paths}")
...
def create_material(textures: dict):
    ...
    material_path = f"{MATERIAL_DIR}/M_PLAYER_M_Meshy_v0_1"
    instance_path = f"{MATERIAL_DIR}/MI_PLAYER_M_Meshy_v0_1"
    if unreal.EditorAssetLibrary.does_asset_exist(material_path):
        unreal.EditorAssetLibrary.delete_asset(material_path)
    material = tools.create_asset(
        "M_PLAYER_M_Meshy_v0_1", MATERIAL_DIR, unreal.Material, unreal.MaterialFactoryNew()
    )
...
def assign_material(mesh, instance) -> int:
    ...
    for item in materials:
        item.set_editor_property("material_interface", instance)
    ...
    for method_name, key in (("get_play_length", "play_length_seconds"), ("get_number_of_sampled_keys", "sampled_keys")):
        ...
        if method:
            try:
                ...
                pass
    for prop, key in (("enable_root_motion", "enable_root_motion"), ("force_root_lock", "force_root_lock")):
        try:
            info[key] = sequence.get_editor_property(prop)
        except Exception:
            pass
    return info
...
def main() -> None:
    ...
    working = project_root / "SourceArt" / "StageG1B" / "PLAYER_M" / "Meshy" / "v0.1" / "Working"
    base_fbx = working / "FBX" / "PLAYER_M_Meshy_v0.1_Base_Rigged.fbx"
    animation_fbx = working / "FBX" / "PLAYER_M_Meshy_v0.1_AllAnimations.fbx"
    ...
    if unreal.EditorAssetLibrary.does_directory_exist(ROOT):
        ...
        remaining = unreal.EditorAssetLibrary.list_assets(ROOT, recursive=True, include_folder=False)
        if remaining:
            raise RuntimeError(f"Unable to clear partial Stage G-1B assets: {len(remaining)}")
    for folder in (MESH_DIR, SKELETON_DIR, TEXTURE_DIR, MATERIAL_DIR, ANIMATION_DIR, f"{ROOT}/AnimationBlueprint", f"{ROOT}/Blueprints"):
    ...
    if mesh is None:
        ...
        if not meshes:
            raise RuntimeError(f"Base skeletal mesh import failed: {initial_task.imported_object_paths}")
        mesh = meshes[0][1]
    ...
    if current_skeleton_path != target_skeleton_path:
        ...
        if not unreal.EditorAssetLibrary.rename_asset(current_skeleton_path, target_skeleton_path):
            raise RuntimeError(f"Unable to move skeleton: {current_skeleton_path}")
        ...
        if physics is not None:
            ...
            if current_physics_path != target_physics_path:
                ...
                if not unreal.EditorAssetLibrary.rename_asset(current_physics_path, target_physics_path):
                    raise RuntimeError(f"Unable to move physics asset: {current_physics_path}")
                physics = unreal.EditorAssetLibrary.load_asset(target_physics_path)
    if physics is None:
        raise RuntimeError("Physics asset was not generated")

    texture_root = base_fbx.parent.parent / "Textures"
    textures = {
        "base": configure_texture(texture_root / "T_PLAYER_M_BaseColor_2K.png", "T_PLAYER_M_BaseColor_2K", True, unreal.TextureCompressionSettings.TC_DEFAULT),
        "normal": configure_texture(texture_root / "T_PLAYER_M_Normal_2K.png", "T_PLAYER_M_Normal_2K", False, unreal.TextureCompressionSettings.TC_NORMALMAP),
        "metallic": configure_texture(texture_root / "T_PLAYER_M_Metallic_2K.png", "T_PLAYER_M_Metallic_2K", False, unreal.TextureCompressionSettings.TC_MASKS),
        "roughness": configure_texture(texture_root / "T_PLAYER_M_Roughness_2K.png", "T_PLAYER_M_Roughness_2K", False, unreal.TextureCompressionSettings.TC_MASKS),
    }
    ...
    if not sequences:
        raise RuntimeError(f"No AnimSequence imported: {animation_task.imported_object_paths}")
    inventory = []
    ...
    (reports / "texture_material_evidence.json").write_text(
        json.dumps(
            {
                "material": material.get_path_name(),
                "material_instance": instance.get_path_name(),
    ...
    log(
        ...
        f"height={final_height:.3f} scale={import_scale:.6f} animations={len(inventory)}"
    )
