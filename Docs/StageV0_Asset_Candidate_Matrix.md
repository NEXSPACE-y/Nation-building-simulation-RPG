...
## 7. 2026-07-17調査時点の構成A・B・C比較（履歴・現在無効）

本節は2026-07-17調査時点の比較履歴である。構成A／B／Cは現在の採用案、取得案、実装案ではない。F4を含む構成Aは、F4外観Pre-Gate不合格により現在無効である。現在状態は§13を正本とする。
...
## 12. 2026-07-17時点の豆虎Gate裁定待ち（履歴）

本節の既存項目は2026-07-17時点の裁定待ち履歴として保持する。現在の正本は§13である。

| 当時の項目 | 現在状態 |
|---|---|
| F1 | 特別保留を継続。現在の次工程ではない |
| F4 | 解消済み。Active候補から除外し、取得・import・使用禁止。P3／P4比較も現在の次工程ではない |
| 構成A／B／C | 現在無効。Stage V-2完了前の詳細確認対象ではない |
| Fab候補確認 | 将来、対象候補がActiveになった場合だけ実施する |
| Git LFS | 現在未導入。別Stage裁定まで停止 |
| キャラクター並置試験 | Stage V-2の対象外。別工程裁定待ち |

以下は2026-07-17時点の原記録であり、現在の実行指示ではない。
...
## 13. Stage V-1以降の豆虎Gate裁定反映

既存候補行、価格、技術情報、当時の`UNKNOWN`、機械評価点、構成A／B／Cは2026-07-17調査時点の比較資料として保持する。次表は後続裁定だけを追補するものであり、既存記録を上書きしない。

| ID | 2026-07-17調査時状態 | 現在状態 | 取得 | import | 使用 | 備考 |
|---|---|---|---|---|---|---|
| F5 | 街路・広場Material候補、UE5.8未import | 正式合格、基準点固定済み | 完了 | 完了 | Stage V-1検証Mapで合格済み。後続PoCは既存Material／MI参照のみ | commit `f08a16c489bdfdaed2e118f99c989ae88728c026`、tag `stage-v1-f5-paving-stones149-v0.1.0`。再取得・再import・再検証禁止。街路表面Materialのみで、街路geometryは未完成 |
| F10 | 屋根Material候補 | 候補として保持、対象屋根Mesh等の確定待ち | 停止 | 未実施 | 未使用 | 対象建築、屋根Mesh、勾配、UV、Material Slot確定まで停止 |
| F11 | 壁Material候補 | 候補として保持、対象壁Mesh等の確定待ち | 停止 | 未実施 | 未使用 | 対象建築、壁Mesh、壁構成、UV、Material Slot確定まで停止 |
| F4 | 王城・城壁候補、外観目視裁定待ち | 調査履歴として保持、Active候補から除外 | 取得禁止 | import禁止 | 使用禁止 | 外観Pre-Gate不合格。LEGO／Minecraft／Primitive／直方体中心の外観riskが正式方針と衝突 |
| F6 | 市場小物候補 | 後続順序再設計まで停止 | 停止 | 未実施 | 未使用 | 対象導線と実装順の再裁定待ち |
| F2 | module比較候補 | 後続順序再設計まで停止 | 停止 | 未実施 | 未使用 | 正式外観候補への自動昇格なし |
| F8 | 植生候補 | 後続順序再設計まで停止 | 停止 | 未実施 | 未使用 | 王城前広場の構造と配置上限が未確定 |
| F9 | 植生候補 | 後続順序再設計まで停止 | 停止 | 未実施 | 未使用 | F8と混在させず、順序再裁定まで停止 |

構成A／B／Cと機械評価は正式採用案、取得順、実装順ではない。候補IDの番号も採用、取得許可、実装順を意味しない。候補検証順と実装順を分離し、実装順は対象導線と対象構造物の完成順で決定する。Gate A／B／CはAsset検査工程であり、王都南門や王城正門などの物理的な門ではない。次工程は`Stage V-2 王都主要街路Geometry PoC`であり、今回の追補は新規Assetの検索・取得を許可しない。
