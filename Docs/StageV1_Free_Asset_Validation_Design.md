...
- 作業種別：Stage V-1設計と実績を保持する現行基準文書
- F5 `Paving Stones 149`：取得、Gate A／B／C、隔離import、実機検証、豆虎Gate合格、正式基準点固定まで完了
- F5以外のAsset取得／import：未実施または停止中
- 現在の次工程：`Stage V-2 王都主要街路Geometry PoC`
- 豆虎Gate：F5の外観裁定と基準点固定完了。後続候補は対象構造物と実装順の再裁定待ち

F5を再取得、再import、再検証しない。F5の合格は単一地面Materialの範囲に限られ、街路geometry、縁石、排水溝、段差、建物際処理、曲がり、交差・分岐、Height／Displacement、Variation、正式王都Map採用を意味しない。本書は後続Assetを取得・importする許可ではない。
...
- 候補ごとの取得前提、保留条件、および実装順から独立した検証手順
- Asset取得前Gate
...
- branch：`main`
- HEAD：`f08a16c489bdfdaed2e118f99c989ae88728c026`
- origin/main：`f08a16c489bdfdaed2e118f99c989ae88728c026`
- annotated tag：`stage-v1-f5-paving-stones149-v0.1.0`
- 開始時git status：clean

上位ビジュアル要件基準点 Stage V-0：

- commit：`3ff9394d2fe8e57c2fb425ae84c52fdd90b311c6`
- annotated tag：`stage-v0-visual-direction-v0.1.0`

Stage V-0基準点は上位要件の履歴として保持し、現在の正式実装基準点と混同しない。
...
用語は`Docs/StageV0_Visual_Asset_Requirements.md`の15.3を正本とする。Gate A／B／CはAsset検査工程、F1～F11はAsset候補IDである。王都南門は王都入口側の物理門、王都城壁は王都外周側の城壁、王城正門は王城前広場から王城へ入る物理門、王城正門門楼は門洞上部と周辺建築、王城正門左右塔は正門左右の塔、王城外郭城壁は王城敷地を囲う城壁、王城中央棟は王城正面奥の主要高層建築を指す。「門」「城門」「屋根」「壁」だけの実装指示は禁止する。
...
| 現在状態 | 候補 | Stage V-1以降の扱い |
|---|---|---|
| 完了 | F5 Paving Stones 149 | 正式合格・基準点固定済み。再取得・再import・再検証禁止。既存Material／MIの参照利用だけを許可 |
| 停止 | F10 Roofing Tiles 011 A | 対象建築、屋根Mesh、勾配、UV、Material Slot確定まで停止 |
| 停止 | F11 Plaster 002 | 対象建築、壁Mesh、壁構成、UV、Material Slot確定まで停止 |
| Inactive | F4 Modular Fantasy Castle | 外観Pre-Gate不合格。Active候補から除外し、取得、import、使用を禁止。過去の調査記録だけを保持 |
| 停止 | F6 Medieval Market / Tent Prop_01 | 新しい実装順が確定するまで停止 |
| 停止 | F2 Medieval Village MegaKit | 新しい実装順が確定するまで停止。正式外観適合を前提にしない |
| 停止 | F8 Free Shrubs Pack | 新しい実装順と配置対象が確定するまで停止 |
| 停止 | F9 Free Nanite Tree | 新しい実装順と配置対象が確定するまで停止 |
...
F1～F11はAsset候補IDであり、採用、取得許可、実装順を意味しない。候補検証順と実装順は分離し、実装順は対象導線と対象構造物の完成順で決定する。

## 7. 検証の原子単位

一つの候補について、外観Pre-Gate、取得経路別Gate、共通の実機検証を一つの連続した原子単位とする。

Fab／ライブラリ型：

1. 外観Pre-Gate
2. Gate A
3. 豆虎Gateによる無料ライブラリ取得。実ファイルはまだダウンロードしない
4. Gate B
5. 豆虎Gateが承認した実ファイルのダウンロード
6. Gate C

ambientCG等の直接ダウンロード型：

1. 外観Pre-Gate
2. Gate A
3. 公式Assetページ上でGate B相当を確認
4. 豆虎Gateによる直接ダウンロード承認と操作
5. Gate C

直接ダウンロード型の無料ライブラリ取得は`N/A`とする。Gate C以降は、原本保全とSHA-256、候補専用Folderへの単独import、専用Mapでの技術・尺度・Camera・性能検証、固定6 Shot、豆虎Gate外観裁定、合格候補の保全または不合格候補のProject内撤去を共通工程とする。

候補Aの裁定と保全／撤去が完了するまで候補Bをimportしない。

## 8. Phase順序

候補検証順と実装順を分離する。旧順序`F5 → F10 → F11 → F4 → F6 → F2 → F8 → F9`は現在の実装順として使用しない。実装順は対象導線と対象構造物の完成順で豆虎Gateが裁定する。

### Phase 0：取得経路別Gate

- Fab／ライブラリ型：外観Pre-Gate → Gate A → 無料ライブラリ取得 → Gate B → ダウンロード → Gate C。
- ambientCG等の直接ダウンロード型：外観Pre-Gate → Gate A → Assetページ上のGate B相当 → 豆虎Gateによる直接ダウンロード → Gate C。
- 直接ダウンロード型の無料ライブラリ取得は`N/A`とする。
- 各Gateの必須項目に`UNKNOWN`または`FAIL`が残る、手順を飛ばす必要がある、ダウンロード後に100MB以上の単体fileを検出した場合は停止する。

### Phase 1：F5完了／F10・F11停止

- F5は2K Textureで全サイクルを完了し、commit `f08a16c489bdfdaed2e118f99c989ae88728c026`、tag `stage-v1-f5-paving-stones149-v0.1.0`として正式基準点固定済みである。
- F5は再取得、再import、再検証しない。単一地面Materialだけが合格範囲であり、街路geometryは未完成である。
- F10は対象建築、屋根Mesh、勾配、UV、Material Slotが確定するまで停止する。
- F11は対象建築、壁Mesh、壁構成、UV、Material Slotが確定するまで停止する。
- Material候補の検証を再開する場合も2K方針を維持し、対象Mesh確定後に候補単位でGateを再開する。

### 廃止：旧Phase 2（F4）

F4 `Modular Fantasy Castle`はLEGO／Minecraft／Primitive／直方体中心の外観riskが正式ビジュアル要件と衝突し、外観Pre-Gate不合格となった。Active候補から除外し、取得、import、使用を行わない。過去の調査記録は削除せず、調査履歴として保持する。

### 停止：旧Phase 3～5（F6／F2／F8／F9）

F6、F2、F8、F9は、新しい実装順と対象導線が確定するまで無料ライブラリ取得、直接ダウンロード、importを行わない。旧Phase番号は調査履歴上の識別にのみ使用し、実装順を意味しない。

### 次工程：Stage V-2 王都主要街路Geometry PoC

王都南門の王都内側接続部から主要街路を経て市場入口手前までについて、既存PoC／Assetの再利用可否と街路geometry方式を先に設計する。Stage V-2の実装方式裁定前に後続Assetを検索・取得しない。
...
8. F2/F6等の既存無料候補では建築品質が不足したという記録
9. 豆虎GateのF1個別承認
...
### 9.1 Gate A：取得または直接ダウンロード前

外観Pre-GateがGOの場合だけ次を確認し、`UNKNOWN`または`FAIL`が一つでも残れば無料ライブラリ取得または直接ダウンロードへ進まない。
...
- 取得経路がFab／ライブラリ型か、ambientCG等の直接ダウンロード型か
- Fab型では豆虎Gateによる無料ライブラリ取得の明示承認
- 直接ダウンロード型では豆虎GateによるAssetページ確認と直接ダウンロードの明示承認

### 9.2 Gate B：取得経路別のダウンロード前確認

Fab／ライブラリ型は、Gate A PASS後に豆虎Gateが無料ライブラリ取得を行う。この時点では実ファイルをダウンロードしない。ライブラリまたはLauncherの表示から次を確認する。
...
ambientCG等の直接ダウンロード型は無料ライブラリ取得を`N/A`とし、公式Assetページで表示容量、取得形式、解像度、配布file構成、依存関係、必要空き容量をGate B相当として確認する。Gate B相当GO後、豆虎Gateが直接ダウンロードを承認・操作する。
...
- 公式掲載画像による外観Pre-Gate裁定
- Fab取得画面の表示確認
- License種別と提供tierの確認
- Gate A後の無料ライブラリ取得ボタンの操作
- Gate B後の実ファイルダウンロード承認と操作
- ambientCG等の公式Assetページ上のGate B相当確認と直接ダウンロード承認・操作
- 有料購入判断
- 取得対象、解像度、容量例外の個別承認
- 外観・操作感の最終判定
...
CodexはFabの契約、License選択、取得、購入操作、および直接ダウンロード操作を代行しない。
...
F4はInactiveであり、新しい外部保管先を作成して取得へ進まない。既存の過去調査証拠がある場合だけ履歴として保持する。
...
上記は現在実在する正式合格候補だけを示す。F10／F11は対象Mesh確定まで、F6／F2／F8／F9は実装順再裁定までFolderを作成しない。F4はActive候補除外済みでありFolderを作成しない。
...
| Prefix | 用途 | 例 |
|---|---|---|
| `SM_` | Static Mesh | `<CandidateID>_SM_<TargetPart>` |
...
| `DA_` | Data Asset | `<CandidateID>_DA_VendorAssetMap` |

Vendor原名は改名対応表へ必ず残す。Vendor Asset本体の一括改名で参照が壊れる場合は原名を維持し、検証用wrapperだけに候補IDを付ける。

命名例は取得、import、実装順の承認を意味しない。F5以外は各停止条件が解除されるまで新規Assetを作成しない。
...
同一の検証用平面、垂直壁、45度勾配屋根を候補種別に応じて使用する。現在はF5の水平・垂直400cm基準面だけが正式基準点に含まれる。

- F5石畳
- F10屋根瓦（対象屋根Mesh確定まで停止）
- F11漆喰（対象壁Mesh確定まで停止）
...
- 上位ビジュアル要件基準はStage V-0 commit `3ff9394d2fe8e57c2fb425ae84c52fdd90b311c6`、現在の正式実装基準はStage V-1／F5 commit `f08a16c489bdfdaed2e118f99c989ae88728c026`とする。
...
- F5は2K Texture、単一地面Material、Material Instance 1件、Tiling 1.0で正式合格し、commit `f08a16c489bdfdaed2e118f99c989ae88728c026`、tag `stage-v1-f5-paving-stones149-v0.1.0`として基準点固定済みである。
- F5は再取得、再import、再検証しない。街路geometry、縁石、排水溝、段差、Height、Variation、正式王都Map採用は合格範囲外である。
- 候補別local branch、50MB以上100MB未満の事前報告、100MB以上での停止、Git LFS未導入を維持する。
- 候補検証順と実装順を分離する。
- F10／F11は対象建築、対象Mesh、勾配または壁構成、UV、Material Slot確定まで停止する。
- F4は外観Pre-Gate不合格としてActive候補から除外し、取得、import、使用を禁止する。過去調査記録は保持する。
- F6／F2／F8／F9は新しい実装順の再裁定まで停止する。
- 次工程は`Stage V-2 王都主要街路Geometry PoC`である。

残裁定：

1. Stage V-2でG-3Aの中心線・尺度だけを再利用するか、既存Primitive geometryを一時利用するか、新規モジュラーまたはSpline方式を採用するか。
2. Stage V-2完了後、対象導線と対象構造物の完成順に基づいてF6／F2／F8／F9の実装順をどう再設計するか。
3. 対象屋根Meshと対象壁Mesh確定後、F10／F11の検証を再開するか。

## 23. 設計時点のUNKNOWN

1. Stage V-2主要街路Geometryの正式実装方式、曲がり半径、分岐形式、縁石・排水溝・建物際処理の最終寸法。
2. G-3AのPoC道路・広場縁石、G-3B-Rの王城用階段moduleを正式街路geometryへ再利用できるか。専用Mesh path、Collision、UV、Material Slotの一部は読取証拠だけでは未確定である。
3. F10の対象建築、屋根Mesh、勾配、UV、Material Slotと、F11の対象建築、壁Mesh、壁構成、UV、Material Slot。
4. F6／F2／F8／F9の新しい実装順、対象導線、最大配置数、取得承認。
5. F1のplaceholder解消、必要subset抽出可否、約12GB保管の個別承認。

F5の取得、import、実機適合、Adapter、性能、外観裁定、基準点は`UNKNOWN`ではなく完了済みである。残る項目は該当Stageまたは候補別Gateで解消し、推測で補完しない。
